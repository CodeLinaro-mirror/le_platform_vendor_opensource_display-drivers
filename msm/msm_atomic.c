...
retry:
		...
		if (!next_injection) {
			...
			next_injection = (prandom_u32() % 40) + 10;
		}
		...
		if (rc == -EDEADLK) {
			 ...
			 */
			if (elapsed_us < RELOCK_MUTEX_RETRY_LIMIT_US) {
				 ...
				 */
				pr_warn("Failed to lock conn_mutex after %lluus, crtc_mask %X\n",
						elapsed_us, crtc_mask);
				ret = rc;
			}
		} else {
			WARN_ON(rc < 0);
