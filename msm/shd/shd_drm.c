		...
		struct drm_display_mode *mode)
{
	...
	ret = msm_property_atomic_set(&sde_crtc->property_info, &cstate->property_state, drm_prop,
				      core_clk);
	if (ret)
		return ret;
...
static int shd_display_atomic_check(struct msm_kms *kms, struct drm_atomic_state *state)
{
	...
	if (base_mask) {
		...
		wait_event_interruptible_locked(priv->pending_crtcs_event, !(priv->pending_crtcs
						& base_mask));
