...
void _sde_shd_hw_ctl_clear_all_blendstages(struct sde_hw_ctl *ctx)
{
	...
	for (i = 0; i < ctx->mixer_count; i++)
		_sde_shd_hw_ctl_clear_blendstages_in_range(hw_ctl,
						ctx->mixer_hw_caps[i].id);
}
...
static inline
void _sde_shd_setup_mixer_out(struct sde_hw_mixer *ctx, struct sde_hw_mixer_cfg *cfg)
{
	/* do nothing */
}
